...
class OnkyoAccessory {
  constructor(platform, receiver) {
    ...
    this.log.info(
      "  GitHub: https://github.com/ToddGreenfield/homebridge-onkyo ",
    );
    this.log.info(
      "**************************************************************",
    );
  ...
  setUp() {
    ...
    this.UUID = this.platform.api.hap.uuid.generate(
      "homebridge:homebridge-onkyo" + this.name,
    );
  ...
  createRxInput() {
    ...
    for (exkey in eiscpData) {
      ...
      if (exkey === "“26”") exkey = "26";

      if (exkey in eiscpData) {
        if ("models" in eiscpData[exkey]) set = eiscpData[exkey].models;
        else continue;
      } else {
        continue;
      }
  ...
  setPowerState(powerOn, callback, context) {
    ...
    if (powerOn) {
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].power + "=on",
        (error, _) => {
          ...
          if (error) {
            ...
            this.log.debug(
              "Attempting to set the default volume to " + this.defaultVolume,
            );
            ...
            if (this.inputs) {
              for (const [_, input] of this.inputs.entries()) {
                if (input.input_name === this.default) label = input.input_name;
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].power + "=standby",
        (error, _) => {
  ...
  getPowerState(callback, context) {
    ...
    this.eiscp.command(
      this.zone + "." + this.cmdMap[this.zone].power + "=query",
      (error, _) => {
        if (error) {
          this.state = false;
          this.log.debug(
  ...
  getVolumeState(callback, context) {
    ...
    this.eiscp.command(
      this.zone + "." + this.cmdMap[this.zone].volume + "=query",
      (error, _) => {
        if (error) {
          this.v_state = 0;
          this.log.debug(
  ...
  setVolumeState(volumeLvl, callback, context) {
    ...
    if (this.mapVolume100) {
      ...
      this.log.debug(
        "setVolumeState - VOLUME LEVEL of: %s exceeds maxVolume: %s. Resetting to max.",
        volumeLvl,
        this.maxVolume,
      );
    } else {
    ...
    this.eiscp.command(
      this.zone + "." + this.cmdMap[this.zone].volume + ":" + this.v_state,
      (error, _) => {
        if (error) {
          this.v_state = 0;
          this.log.debug(
  ...
  setVolumeRelative(volumeDirection, callback, context) {
    ...
    if (volumeDirection === Characteristic.VolumeSelector.INCREMENT) {
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].volume + ":level-up",
        (error, _) => {
          if (error) {
            this.v_state = 0;
            this.log.error(
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].volume + ":level-down",
        (error, _) => {
          if (error) {
            this.v_state = 0;
            this.log.error(
  ...
  getMuteState(callback, context) {
    ...
    this.eiscp.command(
      this.zone + "." + this.cmdMap[this.zone].muting + "=query",
      (error, _) => {
        if (error) {
          this.m_state = false;
          this.log.debug(
  ...
  setMuteState(muteOn, callback, context) {
    ...
    if (this.m_state) {
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].muting + "=on",
        (error, _) => {
          if (error) {
            this.m_state = false;
            this.log.error(
      ...
      this.eiscp.command(
        this.zone + "." + this.cmdMap[this.zone].muting + "=off",
        (error, _) => {
          if (error) {
            this.m_state = false;
            this.log.error(
  ...
  getInputSource(callback, context) {
    ...
    this.eiscp.command(
      this.zone + "." + this.cmdMap[this.zone].input + "=query",
      (error, _) => {
        if (error) {
          this.i_state = 1;
          this.log.error(
  ...
  createTvSpeakerService(tvService) {
    this.tvSpeakerService = this.accessory.addService(
