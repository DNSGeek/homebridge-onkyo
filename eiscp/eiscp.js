...
function command_to_iscp(command, args, zone) {
  ...
  if (typeof VALUE_MAPPINGS[zone][prefix][args] === "undefined") {
    if (
      typeof VALUE_MAPPINGS[zone][prefix].INTRANGES !== "undefined" &&
      /^\d+$/.test(args)
    ) {
      ...
      for (i = 0; i < len; i += 1) {
        if (
          in_modelsets(intranges[i].models) &&
    ...
    if (
      ...
      in_modelsets(VALUE_MAPPINGS[zone][prefix][args].models)
    ) {
...
self.connect = function (options) {
  ...
  config.reconnect =
    options.reconnect === undefined ? config.reconnect : options.reconnect;
  config.reconnect_sleep = options.reconnect_sleep || config.reconnect_sleep;
  config.verify_commands =
    options.verify_commands === undefined
      ? config.verify_commands
...
self.get_commands = function (zone, callback) {
  ...
  var result = [];
  async.each(
    Object.keys(COMMAND_MAPPINGS[zone]),
    function (cmd, cb) {
      //console.log(cmd);
      result.push(cmd);
      cb();
...
self.get_command = function (command, callback) {
  ...
  async.each(
    Object.keys(VALUE_MAPPINGS[zone][COMMAND_MAPPINGS[zone][command]]),
    function (val, cb) {
      result.push(val);
      cb();
    },
